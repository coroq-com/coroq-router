...
A key that looks like a number does not do what it looks like. To route `/archive/2026` to a controller, writing

```php
$routeMap = [
    'archive' => [
        '2026' => Controller\Archive\Year2026::class,
        'latest' => Controller\Archive\LatestController::class,
    ],
];
```

does not work. PHP converts the key `'2026'` into the integer `2026`, so the controller becomes a numeric entry: it is collected on the way like middleware, and `/archive/2026` matches nothing.

```php
$router->route(['archive', '2026']);    // throws RouteNotFoundException
$router->route(['archive', 'latest']);  // [Year2026::class, LatestController::class]
```

Use `SegmentRouter` for such segments:
...
It matches one segment against a fixed value, and reads that value the way a route map does, so a nested array or another router works there too.
